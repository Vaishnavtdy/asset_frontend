...
export default {
  ...
  async mounted() {
    ...
    //console.log(this.$page.strapi.tags[0].slug, "slug");
    let posts = await fetch(
      process.env.GRIDSOME_STRAPI_URL +
        "/properties?tags.label_contains=" +
        slug
    ).then((res) => res.json());

    this.posts = JSON.parse(JSON.stringify(posts));
    console.log(this.$page.strapi.tags[0].metaTitle);
    //console.log(this.posts,"thisposts");
    return this.posts;
  },
  methods: {
    getStrapiMedia,
  ...
  metaInfo() {
    ...
    return {
      title: this.$page.strapi.tags[0].metaTitle,
